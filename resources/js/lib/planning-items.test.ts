...
describe('planningAttentionTier', () => {
    ...
    // 90%, not 80%: budgetSeverity() reads the same status the server does
    // through BudgetPeriod::CLOSE_TO_LIMIT_THRESHOLD_PERCENTAGE, so the tier a
    // budget takes here matches the point at which it emails the user.
    it('puts a budget close to its limit in the middle tier', () => {
        expect(
            planningAttentionTier(item(budget('a', 100, 90), 'budget')),
        ).toBe(1);
        expect(
            planningAttentionTier(item(budget('a', 100, 89), 'budget')),
        ).toBe(2);
    });
