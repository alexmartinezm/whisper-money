 ...
 *
 * The result is stored on the summary, so opening the month again does not pay
...
class AnalysisWriter
{
     ...
     * month's figures are shown either way.
     */
     ...
     * it is worth a few attempts. What it is never worth is holding up the
     * figures: once the attempts are spent this returns null and the month is
     * shown without the section.
     *
     * Nothing is written here: the caller decides whether the text is kept.
     */
    public function draft(MonthlySummary $summary, User $user): ?string
    {
        ...
        for ($attempt = 1; $attempt <= $attempts; $attempt++) {
            try {
                ...
                // reachable is expected, not a bug. Back off and try again;
                // only give up once the attempts run out.
        ...
        // an outage that just cost a reader their month, and with logs off the
        // per-attempt warning only reaches Sentry as a breadcrumb on somebody
        // else's event — so a wrong endpoint or a revoked key surfacing as
        // connect failures would otherwise go dark. Getting here means the last
        // attempt was one of those failures.
    ...
    private function promptOnce(MonthlySummary $summary, User $user): ?string
    {
        ...
        // The answer is rendered as paragraphs of plain text, so markdown
     ...
     * the screen does rather than as bare floats.
     */
     ...
     * elsewhere — their region's way, which is not their language's: the model
     * is told to write in Spanish and handed Mexican numbers. Nothing is added
     * that is not already in the payload, which is what keeps the promise stated
     * beside the block true.
     */
