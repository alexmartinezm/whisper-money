...
class SummaryBuilder
{
    ...
    public function build(User $user, Carbon $month, bool $complete): array
    {
        ...
        return [
            ...
            'net_worth' => $this->netWorthSection($accounts, $lookup, $month, $currency),
            'cashflow' => $cashflow,
     ...
     * @return array<string, mixed>
     */
    private function netWorthSection(Collection $accounts, BalanceLookup $lookup, Carbon $month, string $currency): array
    {
        // What counts is already decided per account by `include_in_net_worth`,
        // which is finer than a per-type preference, so the calculator needs no
        // exclusion list: this quotes the same total the dashboard headline and
        // the get_net_worth MCP tool report.
        $history = [];
        ...
        for ($ago = self::HISTORY_MONTHS; $ago >= 0; $ago--) {
            ...
            $history[] = [
                ...
                'value' => $this->netWorthCalculator->at($accounts, $lookup, $end, $currency),
            ];
        }
