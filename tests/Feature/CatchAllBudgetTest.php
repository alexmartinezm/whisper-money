...
test('historical assignment keeps an expense whose label budget has no period covering it', function () {
    ...
    $transaction = Transaction::factory()->create(['user_id' => $this->user->id,
        'space_id' => $this->user->personalSpace->id, 'category_id' => $category->id, 'transaction_date' => now()->subDay(), 'amount' => -1000]);
    $transaction->labels()->attach($label);
...
test('historical assignment skips expenses whose label another budget tracks', function () {
    ...
    $labelled = Transaction::factory()->create(['user_id' => $this->user->id,
        'space_id' => $this->user->personalSpace->id, 'category_id' => $category->id, 'transaction_date' => now()->subDay(), 'amount' => -1000]);
    $labelled->labels()->attach($label);
    Transaction::factory()->create(['user_id' => $this->user->id,
        'space_id' => $this->user->personalSpace->id, 'category_id' => $category->id, 'transaction_date' => now()->subDay(), 'amount' => -2000]);
